...
| `exec_command` | `exec` | 每次问 |
| `code_interpreter` | `exec` | 每次问 |
| 天气/时间/汇率 | `none` | 不问 |

**只门禁文件三件套等于没门禁**：`exec_command` 能 `cat`/`rm` 任意路径，`code_interpreter` 能 `open()` 任意文件，两者都会绕过文件工具。这与 ADR 0005 D1 记的是同一个洞。
...
- `exec_command`：命令原文 + `description`（D5 预留的字段，这里的第 1 个真实消费者）；
...
用户能看到并控制 Agent 的**意图**，但一旦放行，被放行的动作本身不受任何新增限制。`exec_command` 被批准一次就等于批准任意文件写入。因此：
...
- 工具描述里的"Does not pause execution"必须删改，否则模型会基于假信息决定是否调用（尤其 `exec_command` 的 `description` 参数——它现在真的会被展示）。
...
- **只门禁文件三件套**：`exec_command` 与 `code_interpreter` 一步绕过，且界面会给人"已保护"的错觉。
...
> **补充（[ADR 0008](0008-write-stdin-tool.md)，2026-09-23）**：可选答案不再对每个工具都一样。`ToolSpec.scopes` 可以收窄一个工具接受的 scope，权限请求把 `scopes` 一并交给前端，对话框只渲染列出的按钮；API 收到工具不接受的 scope 返回 400，而不是静默降级。首个用例是 `write_stdin`——它的目标是一次性会话 id，持久化"always"规则永远匹配不上，只会留下垃圾规则。
