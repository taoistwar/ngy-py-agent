...
决定（选 b）：`exec_command` 与 `code_interpreter` 并存，后者的问题后续专门处理。

后果必须写明，不能让读者以为这套边界是完整的：

- `exec_command` 的进程树/资源约束**只约束 `exec_command`**。模型用 `code_interpreter` 一步即可绕过工作空间约束、访问策略、编码保证与并发复核。
- 因此本 ADR 中出现"containment"一词时，指的是**资源与进程生命周期**，不是文件或网络隔离。
- 相关工具的用户可见文案不得宣称"已沙箱化"。`exec_command` 在结果里固定上报 `containment.note`，原文包含 "not a security boundary"。
...
> **已被 [ADR 0006](0006-tool-permission-confirmation.md) 部分取代（2026-09-20）**：P1 交互式确认落地后，D5 预言的第 1 个消费者（**权限对话框展示 `description`**）已经存在，且 `exec_command` 会真的暂停等待答复。因此本节"工具描述不写'给权限系统看'"以及 `exec_tool.py` 里"it does not pause execution"的措辞**都已不成立并被修改**。仍成立的部分：分类器与 `allow` 规则生成未实现，持久化规则见 ADR 0006 D8。
...
> **已被 [ADR 0008](0008-write-stdin-tool.md) 修订（2026-09-23）**：`run_in_background` 现在返回可交互的 `session_id`，句柄改由 `process_store` 持有，且**会话在任务结束时会被停止**——"不保证回收"变成了"任务结束即回收"，另有每任务容量上限与 TTL 兜底。本节关于 `timeout`、`interrupted` 与 PID 文件用途的部分仍然成立。
...
- 命令**不受** `file_access` 策略约束（它就是个 shell）。这是 D1 的直接后果，不要试图在 `exec_command` 上假装有路径管控。
...
- **在 `exec_command` 上套用 `file_access` 的路径策略**：无法强制执行（命令可自行读写任意路径），假装有管控比不做更糟。
